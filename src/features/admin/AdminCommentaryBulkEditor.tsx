...
export function AdminCommentaryBulkEditor({
  ...
  // One textarea per verse — keyed by verse_number
  ...
  // Reset drafts when the chapter changes
  useEffect(() => {
    setDrafts({});
    setResult(null);
    setError(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResult(
        `${response.created} ትርጓሜዎች ተቀምጠዋል${response.skipped ? ` · ${response.skipped} ተዘሏል` : ""}`,
      );
      setDrafts({});
      onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Save bar */}
      <div className="sticky bottom-4 flex items-center justify-between gap-4 rounded-xl border border-gold-500/30 bg-stone-950/95 px-4 py-3 backdrop-blur-lg">
