...
function useSourcesForPicker() {
  return useQuery({
    ...
    queryFn: () =>
      request<{ data: SourceOut[]; meta: { total: number } }>(
  ...
  return (
    <>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <select
            ...
            className="rounded-lg border border-surface-border bg-surface-sunken/40 px-3 py-2 font-amharic text-[13px] text-text-primary focus:border-gold-500/40 focus:outline-none"
          >
      ...
      {isLoading ? (
        ...
        <EmptyState titleAm="ስህተት ተፈጥሯል" hintAm="ትርጓሜዎችን መጫን አልተቻለም።" />
      ) : items.length === 0 ? (
        <EmptyState
          titleAm="ትርጓሜ የለም"
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="1"
            className="w-full rounded-lg border border-surface-border bg-surface-sunken/40 px-3.5 py-2.5 font-mono text-[15px] text-text-primary focus:border-gold-500/40 focus:outline-none"
          />
          <p className="mt-1 text-[11px] text-text-faint">
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-lg border border-surface-border bg-surface-sunken/40 px-3.5 py-2.5 font-amharic text-[15px] text-text-primary focus:border-gold-500/40 focus:outline-none"
          >
            <option value="">—</option>
            {sources.map((s) => (
              <option key={s.id} value={s.id}>
